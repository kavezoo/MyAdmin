/**
 * President members — national fee SWAL + enable/disable AJAX + SWAL.
 */
(function ($) {
	'use strict';
	...
	function updateEnabledUi($btn, enabled) {
		...
		var cfg = window.PresidentMembers || {};
	...
	$(function () {
		...
		$(document).on('click', '.js-record-national-fee', function (e) {
			e.preventDefault();
			var $btn = $(this);
			var memberName = $btn.data('memberName') || '';
			var title = cfg.recordTitle || 'Record national membership fee payment?';
			var text = memberName
				? (cfg.recordTextNamed || 'Do you confirm that {0} has paid the national membership fee for this year? The payment date will be set to today.')
				: (cfg.recordText || 'Do you confirm that this member has paid the national membership fee for this year? The payment date will be set to today.');
