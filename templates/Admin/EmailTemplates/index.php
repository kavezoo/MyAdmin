<?php
/**
 * Email templates index (Admin) — standard Admin CRUD list.
 *
			...
			<div class="card-header">
				...
				<div class="float-right d-flex align-items-center gap-2 flex-wrap justify-content-end">
					...
					<form method="get" action="<?= h($this->Url->build(['action' => 'index'])) ?>"
						...
						<?php endforeach; ?>
						<?php if (!array_key_exists('country_id', $filterQuery)): ?>
							<input type="hidden" name="country_id" value="<?= (int)($filterCountryId ?? 0) ?>">
						<?php endif; ?>
					...
					<form method="get" class="table-search" role="search">
						<input type="hidden" name="country_id" value="<?= $filterCountryId ?>">
						...
						<?php if ($hasSearch): ?>
							<a href="<?= h($this->Url->build(['?' => [
								'clear_search' => '1',
								'country_id' => (string)$filterCountryId,
								'language_id' => (string)$filterLanguageId,
							]])) ?>"
