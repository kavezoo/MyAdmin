 ...
 * @var string $mode profile|profile_club|table|table_action|table_national_action|table_club_national_action
 * @var string $memberName For SWAL (table_action / club name for club national)
...
if ($mode === 'profile' || $mode === 'profile_club'): ?>
	<div class="membership-fee-status membership-fee-status--profile <?= $paid ? 'membership-fee-status--paid' : 'membership-fee-status--unpaid' ?>">
		...
		<?php elseif ($mode === 'profile_club'): ?>
			<div class="membership-fee-status__message membership-fee-status__message--unpaid">
				...
				<span class="d-block mt-1"><?= h(__('The club has not paid the annual national membership fee for {0} yet.', $membershipYear)) ?></span>
				<?php if ($lastPaymentDateFormatted !== ''): ?>
					<span class="d-block mt-1 text-muted"><?= h($lastPaymentHint) ?></span>
				<?php endif; ?>
			</div>
		<?php else: ?>
			<div class="membership-fee-status__message membership-fee-status__message--unpaid">
...
<?php elseif ($mode === 'table_club_national_action'): ?>
	<?php if ($paid): ?>
	...
	<?php else: ?>
		<div class="membership-fee-cell membership-fee-cell--unpaid text-center">
			<button
				type="button"
				class="btn btn-sm btn-danger membership-fee-record-btn js-record-club-national-fee"
				data-form-id="<?= h($formId) ?>"
				data-member-name="<?= h($memberName) ?>"
				title="<?= h(__('Record club national fee payment')) ?>"
			>
